...
from count_useless import count_recursive_rules
...
class EvolutionaryEngine:
	...
	def threaded_function(self, threadNumber, runs):
		try:
			for run in runs:
				...
				os.makedirs(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/initial_pop/')
				os.makedirs(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/fitness/')
				...
				for generation in range(self.config_file.num_generations):
					...
					os.makedirs(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation))
					...
					for pop_element in range(0,len(selection_output),2):
						...
						if first_debug is True and (check_member_validity(A) is False or check_member_validity(B) is False):
						...
						if first_debug is True and (check_member_validity(new_A) is False or check_member_validity(new_B) is False):
							logging.debug('Error checking member validity after crossover')
							first_debug = False
						
						#mutation
						random.setstate(mutation(new_A, self.config_file.mutation_startshape, self.config_file.mutation_background_rename, self.config_file.mutation_background_value, self.config_file.mutation_background_mu, self.config_file.mutation_background_std,
							self.config_file.mutation_parameter_rename, self.config_file.mutation_parameter_remove, self.config_file.mutation_parameter_value, self.config_file.mutation_parameter_mu, 
							self.config_file.mutation_parameter_std, self.config_file.mutation_rule_duplicate, self.config_file.mutation_rule_remove, self.config_file.mutation_rule_copyRename, self.config_file.mutation_symbol_remove,
							self.config_file.mutation_symbol_replace, self.config_file.mutation_symbol_add, random.getstate()))

						random.setstate(mutation(new_B, self.config_file.mutation_startshape, self.config_file.mutation_background_rename, self.config_file.mutation_background_value, self.config_file.mutation_background_mu, self.config_file.mutation_background_std,
							self.config_file.mutation_parameter_rename, self.config_file.mutation_parameter_remove, self.config_file.mutation_parameter_value, self.config_file.mutation_parameter_mu, 
							self.config_file.mutation_parameter_std, self.config_file.mutation_rule_duplicate, self.config_file.mutation_rule_remove, self.config_file.mutation_rule_copyRename, self.config_file.mutation_symbol_remove,
							self.config_file.mutation_symbol_replace, self.config_file.mutation_symbol_add, random.getstate()))
						...
						os.makedirs(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element))
						self.write_file(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element)+'/'+str(run)+'_'+str(generation)+'_'+str(pop_element)+'.cfdg', new_A.printMember())
						os.makedirs(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element+1))
						self.write_file(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element+1)+'/'+str(run)+'_'+str(generation)+'_'+str(pop_element+1)+'.cfdg', new_B.printMember())


						new_A.fileName = self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element)+'/'+str(run)+'_'+str(generation)+'_'+str(pop_element)+'.cfdg'
						new_B.fileName = self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/pop'+str(generation)+'/ind'+str(pop_element+1)+'/'+str(run)+'_'+str(generation)+'_'+str(pop_element+1)+'.cfdg'

						#cfdg rendering to .png
						fitness.renderGrammar(new_A, new_A.fileName, new_A.fileName.split('.cfdg')[0]+'.png', self.config_file.contextfree_seed,rendering_time=self.config_file.rendering_time, max_generations = self.config_file.max_renderer)
						fitness.renderGrammar(new_B, new_B.fileName, new_B.fileName.split('.cfdg')[0]+'.png', self.config_file.contextfree_seed,rendering_time=self.config_file.rendering_time, max_generations = self.config_file.max_renderer)
					...
					new_population = self.elite_selection(current_population, new_population, self.config_file.elite_size)

					fitness.classifyFitness(new_population[:self.config_file.elite_size], self.config_file.fitness_function, True)
					...
					#header
					to_append_write = ''
					if not os.path.isfile(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/fitness/run_statistics.csv'):
						...
						to_append_write += ',Max '+'_'.join(self.config_file.fitness_function)+',[Global] Min '+'_'.join(self.config_file.fitness_function)+',[Local] Min '+'_'.join(self.config_file.fitness_function)+',[Global] AVG '+'_'.join(self.config_file.fitness_function)+',[Local] AVG '+'_'.join(self.config_file.fitness_function)+',[Global] STD '+'_'.join(self.config_file.fitness_function)+',[Local] STD '+'_'.join(self.config_file.fitness_function)
					...
					f_out = open(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/fitness/run_statistics.csv', 'a')
					...
					data_fitness_csv = [["Filename", "# Rules", "Dif Rules","Total Reachable Rules","Total Unreachable Rules","Reachable Rules", "Unreachable Rules","%Total Reachable", "%Total Unreachable", "% Reachable", "% Unreachable", "Productive Rules", "Unproductive Rules", "% Productive", "% Unproductive", '_'.join(self.config_file.fitness_function)]+self.config_file.other_fitnesses]
					...
					write_content(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/fitness/pop'+str(generation)+'.csv', data_fitness_csv, assure_new=True, multiple=True)
	...
	def __init__(self):
		global ACTIVE_MODELS, TARGET_CLASS, NETWORKS
		...
		# TOM WHITE FUNCTIONS STUFF
		parser = argparse.ArgumentParser(description="evolve to objective")
		#parser.add_argument('--save-folder', required=True, help="Directory to experiment outputs (ex: 'experiments'). This is a required argument.")
		...
		parser.add_argument("--nthreads", default=2, type=int, help="comma separated list of networks (no spaces). Default is 2.")
		parser.add_argument("--best_number", default=10, type=int, help="comma separated list of networks (no spaces). Default is 10.")
		...
		self.best_number = args.best_number
		TARGET_CLASS = args.target_class

		ACTIVE_MODELS = get_active_models_from_arg(NETWORKS)
		fitness.ACTIVE_MODELS = ACTIVE_MODELS
		...
		fitness.IMAGENET_INDEXES = IMAGENET_INDEXES
		...
		self.config_file.output_directory=self.config_file.output_directory+'/'+'_'.join(self.config_file.fitness_function)
		
		os.makedirs(self.config_file.output_directory)
		...
		for thread_index in range(self.num_threads):
			t = Thread(target = self.threaded_function, args=(thread_index, range(thread_index*num_runs_per_thread, (thread_index+1)*num_runs_per_thread)))
		...
		#produce  graphics
		#ro.r('source("graphics.R")')
		#r_command = 'plot_csv(x=1:'+str(self.config_file.num_generations)+', file="'+self.config_file.output_directory+'/global.csv'+'", comparable_cols = c(104,108), ylab="",lty = c(1,2), title="Teste", output="'+self.config_file.output_directory+'/graphic.pdf'+'")'
		#ro.r(r_command)
	...
	def generateFinalStats(self):
		...
		def work_in_range(idx_min, idx_max, files_list):
			_max = [[] for x in range(self.config_file.num_generations)]
			global_min = [[] for x in range(self.config_file.num_generations)]
			local_min = [[] for x in range(self.config_file.num_generations)]
			global_mean = [[] for x in range(self.config_file.num_generations)]
			local_mean = [[] for x in range(self.config_file.num_generations)]
			global_std = [[] for x in range(self.config_file.num_generations)]
			local_std = [[] for x in range(self.config_file.num_generations)]
			...
			for gen in range(self.config_file.num_generations):
				output.append([max(_max[gen]), min(global_min[gen]), min(local_min[gen]), numpy.mean(global_mean[gen]), numpy.mean(local_mean[gen]), numpy.mean(global_std[gen]), numpy.mean(local_std[gen])])
		...
		def get_non_renderable(col, files_list):
			...
			for gen in range(self.config_file.num_generations):
				output.append([max(non_renderable[gen]), min(non_renderable[gen]), numpy.mean(non_renderable[gen]), numpy.std(non_renderable[gen])])
		...
		files_list = list()
		for run in range(self.config_file.num_runs):
			folder = self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/fitness/'
		...
		f_out.write('Generation,Max Non-Renderable,Min Non-Renderable,AVG Non-Renderable,STD Non-Renderable,Max Num. Rules, [Global] Min Num. Rules,[Local] Min Num. Rules,[Global] AVG Num. Rules,[Local] AVG Num. Rules,[Global] STD Num. Rules,[Local] STD Num. Rules,Max Dif. Rules, [Global] Min Dif. Rules,[Local] Min Dif. Rules,[Global] AVG Dif. Rules,[Local] AVG Dif. Rules,[Global] STD Dif. Rules,[Local] STD Dif. Rules,Max Total Reachable Rules, [Global] Min Total Reachable Rules,[Local] Min Total Reachable Rules,[Global] AVG Total Reachable Rules,[Local] AVG Total Reachable Rules,[Global] STD Total Reachable Rules,[Local] STD Total Reachable Rules,Max Total Unreachable Rules, [Global] Min Total Unreachable Rules,[Local] Min Total Unreachable Rules,[Global] AVG Total Unreachable Rules,[Local] AVG Total Unreachable Rules,[Global] STD Total Unreachable Rules,[Local] STD Total Unreachable Rules,Max Reachable Rules, [Global] Min Reachable Rules,[Local] Min Reachable Rules,[Global] AVG Reachable Rules,[Local] AVG Reachable Rules,[Global] STD Reachable Rules,[Local] STD Reachable Rules,Max Unreachable Rules, [Global] Min Unreachable Rules,[Local] Min Unreachable Rules,[Global] AVG Unreachable Rules,[Local] AVG Unreachable Rules,[Global] STD Unreachable Rules,[Local] STD Unreachable Rules,Max % Total Reachable, [Global] Min % Total Reachable,[Local] Min % Total Reachable,[Global] AVG % Total Reachable,[Local] AVG % Total Reachable,[Global] STD % Total Reachable,[Local] STD % Total Reachable,Max % Total Unreachable, [Global] Min % Total Unreachable,[Local] Min % Total Unreachable,[Global] AVG % Total Unreachable,[Local] AVG % Total Unreachable,[Global] STD % Total Unreachable,[Local] STD % Total Unreachable,Max % Reachable, [Global] Min % Reachable,[Local] Min % Reachable,[Global] AVG % Reachable,[Local] AVG % Reachable,[Global] STD % Reachable,[Local] STD % Reachable,Max % Unreachable, [Global] Min % Unreachable,[Local] Min % Unreachable,[Global] AVG % Unreachable,[Local] AVG % Unreachable,[Global] STD % Unreachable,[Local] STD % Unreachable,Max Productive,[Global] Min Productive,[Local] Min Productive,[Global] AVG Productive,[Local] AVG Productive,[Global] STD Productive,[Local] STD Productive,Max Unproductive,[Global] Min Unproductive,[Local] Min Unproductive,[Global] AVG Unproductive,[Local] AVG Unproductive,[Global] STD Unproductive,[Local] STD Unproductive,Max % Productive,[Global] Min % Productive,[Local] Min % Productive,[Global] AVG % Productive,[Local] AVG % Productive,[Global] STD % Productive,[Local] STD % Productive,Max % Unproductive,[Global] Min % Unproductive,[Local] Min % Unproductive,[Global] AVG % Unproductive,[Local] AVG % Unproductive,[Global] STD % Unproductive,[Local] STD % Unproductive')
		f_out.write(',Max '+'_'.join(self.config_file.fitness_function)+',[Global] Min '+'_'.join(self.config_file.fitness_function)+',[Local] Min '+'_'.join(self.config_file.fitness_function)+',[Global] AVG '+'_'.join(self.config_file.fitness_function)+',[Local] AVG '+'_'.join(self.config_file.fitness_function)+',[Global] STD '+'_'.join(self.config_file.fitness_function)+',[Local] STD '+'_'.join(self.config_file.fitness_function))
		for other_fit in self.config_file.other_fitnesses:
			f_out.write(',Max '+other_fit+',[Global] Min '+other_fit+',[Local] Min '+other_fit+',[Global] AVG '+other_fit+',[Local] AVG '+other_fit+',[Global] STD '+other_fit+',[Local] STD '+other_fit)

		f_out.write('\n')

		for generation in range(self.config_file.num_generations):
			f_out.write(str(generation)+','+to_string(non_renderable[generation])+to_string(num_rules[generation])+to_string(dif_rules[generation])+to_string(total_reachable[generation])+to_string(total_unreachable[generation])+to_string(reachable[generation])+to_string(unreachable[generation])+to_string(per_total_reachable[generation])+to_string(per_total_unreachable[generation])+to_string(per_reachable[generation])+to_string(per_unreachable[generation])+to_string(productive[generation])+to_string(unproductive[generation])+to_string(per_productive[generation])+to_string(per_unproductive[generation])+to_string(guide_fitness[generation]))
	...
	def open_initial_pop(self,run):
		if self.config_file.initial_pop_directory is None:
			initial_pop=[]
			random_pop = initial_pop_random.generateInitialPopulation(self.config_file.output_directory+'/run_'+str(run)+'_'+self.config_file.seeds[run]+'/initial_pop', self.config_file.fitness_function, self.config_file.pop_size, run, self.config_file.contextfree_seed,self.config_file.seeds[run],self.config_file.max_renderer, self.config_file.rendering_time)
			return random_pop
			...
			for _file in array_files:
				...
				fitness.renderGrammar(pop_member, pop_member.fileName, pop_member.fileName.split('.cfdg')[0]+'.png', self.config_file.contextfree_seed,rendering_time=self.config_file.rendering_time, max_generations = self.config_file.max_renderer)
	...
	def elite_selection(self, old_pop, new_pop, elite_size):
		...
		for current_member_elite in temp_pop[:elite_size]:
			...
			fitness.renderGrammar(current_member_elite, current_member_elite.fileName, current_member_elite.fileName.split('.cfdg')[0]+'.png', self.config_file.contextfree_seed,rendering_time=self.config_file.rendering_time, max_generations = self.config_file.max_renderer)

		return temp_pop	


if __name__ == "__main__":
	#best_number = int(argv[1])
