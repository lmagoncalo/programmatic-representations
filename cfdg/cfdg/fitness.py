...
def renderGrammar(member, origin, destination, contextfree_seed=None, rendering_time=None, max_generations=1000000, width=256, height = 256):	
		...
		if(contextfree_seed is not None):
			if type(contextfree_seed) is str:
				...
				Command('./cfdg '+fitness+' -w '+str(width)+' -h '+str(height)+' -N '+max_generations+' -o '+destination + ' ' + origin, rendering_time)
				...
				for seed in contextfree_seed:
					...
					destination+='/'+split_destination[0]+'_'+split_destination[1]+'_'+split_destination[2]+'_'+seed+'.png'

					Command('./cfdg '+origin+fitness+' -w '+str(width)+' -h '+str(height)+' -N '+max_generations+' -o '+destination, rendering_time)
					member.addRendering(destination)
...
def fractalDimension(files_list):
	return run_frac_lac(fitness_function='frac',files_list=files_list,csv_path='fitnessFrac.csv', del_file=True, objective_value=1.5)
