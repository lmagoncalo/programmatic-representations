...
int Variation::random(int letters)
{
    ...
    if (!seeded) {
		...
		unsigned short randomSeed[3];
		CryptAcquireContext(&csphandle, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT);
		CryptGenRandom(csphandle, 6, (BYTE*)randomSeed);
		CryptReleaseContext(csphandle, 0);
        Rand48::Common.seed(randomSeed);
#else
        /* [AMS] 2/13/2007 -- Added seeding from /dev/urandom */
        unsigned short urandomSeed[3];
        std::ifstream urand("/dev/urandom", std::ios::in | std::ios::binary);
        if (urand.is_open()) {
            urand.read((char*)urandomSeed, 3 * sizeof(unsigned short));
            urand.close();
            Rand48::Common.seed(urandomSeed);
        } else {
            Rand48::Common.setRand((long)time(0));
        }
#endif
        seeded = true;
    }
    ...
    return min + Rand48::Common.getUnsigned() % (max - min + 1);
}
